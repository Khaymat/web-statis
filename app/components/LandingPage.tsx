...
import ProtectedQuotesSection from "./ProtectedQuotesSection"
      ...
      <div ref={containerRef} className="h-full overflow-y-auto snap-y snap-mandatory">
        {sections.map((section, index) => {
          if (section.id === "quotes") {
            return <ProtectedQuotesSection key={section.id} />
          }
          return <Section key={section.id} {...section} isActive={index === activeSection} />
        })}
      </div>
    </Layout>
  )
